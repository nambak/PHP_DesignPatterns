...
* ## 생성패턴
  소프트웨어 엔지니어링에서 생성패턴은 상황에 맞는 방식으로 객체를 생성하려고 시도하면서 객체 생성 메커니즘을 다루는 디자인 패턴입니다. 
  객체 생성의 기본 형태는 디자인 문제를 야기하거나 디자인에 복잡성을 추가 할 수 있습니다. 
  생성패턴은 이 객체 생성을 어떻게 든 제어함으로써이 문제를 해결합니다.
  * ### Abstract Factory:
...
* ## More
  * ### Service Locator
    **이것은 안티 패턴으로 간주됩니다!**
    ...
    Service Locator는 어떤 사람들에게는 안티 패턴으로 간주됩니다. 
    종속성 반전 원칙을 위반합니다. Service Locator는 종속성 주입을 사용하는 것처럼 클래스의 종속성을 노출하는 대신 숨깁니다. 
    이러한 종속성이 변경되면이를 사용하는 클래스의 기능이 손상되어 시스템 유지 관리가 어려워 질 위험이 있습니다.

    * 목적

    더 나은 테스트, 유지 관리 및 확장 가능한 코드를 얻기 위해 느슨하게 결합 된 아키텍처를 구현합니다. 
    DI 패턴 및 서비스 로케이터 패턴은 Inverse of Control 패턴의 구현입니다.

    * 용법

    함께 ServiceLocator사용하면 지정된 인터페이스에 대한 서비스를 등록 할 수 있습니다. 
    인터페이스를 사용하여 서비스를 검색하고 구현을 몰라도 애플리케이션의 클래스에서 사용할 수 있습니다. 
    부트스트랩에서 Service Locator 객체를 구성하고 삽입 할 수 있습니다.
  * ### Repository
    * 목적

    도메인 개체에 액세스하기 위해 컬렉션과 유사한 인터페이스를 사용하여 도메인과 데이터 매핑 레이어 사이를 중재합니다. 
    리포지토리는 데이터 저장소에 유지되는 개체 집합과 개체에 대해 수행되는 작업을 캡슐화하여 지속성 계층에 대한 보다 개체 지향적인 보기를 제공합니다. 
    리포지토리는 또한 도메인과 데이터 매핑 계층 사이에 명확한 분리 및 단방향 종속성을 달성하는 목표를 지원합니다.
